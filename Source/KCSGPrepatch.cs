...
namespace KCSG
{
    ...
    // Core container for our unlimited symbol support - old version
    public static class OldSymbolRegistry
    {
        // Shadow registry for additional symbols beyond the 65535 limit
        public static Dictionary<string, object> ShadowSymbolRegistry = new Dictionary<string, object>();
        public static bool Initialized = false;
        
        // Stats for monitoring
        public static int TotalAdditions = 0;
        public static int TotalRetrievals = 0;
        public static int OverflowAdditions = 0;
        ...
        public static void Initialize()
        {
            ...
            if (Initialized)
            {
                Log.Message("[KCSG] Shadow registry already initialized, skipping");
                return;
            }
            
            Log.Message("[KCSG] Setting up shadow registry system for unlimited symbols");
            Initialized = true;
            
            // Create an empty registry with case insensitive comparison for better compatibility
            ShadowSymbolRegistry = new Dictionary<string, object>(StringComparer.OrdinalIgnoreCase);
            
            Log.Message("[KCSG] Shadow registry system is ready with capacity for unlimited symbols");
        }
        
        // Add a symbol to the shadow registry
        public static void AddSymbol(string name, object symbol)
        {
            if (!Initialized)
            {
                Initialize();
            ...
            {
                Log.Warning($"[KCSG] Symbol '{name}' already exists in shadow registry, replacing");
                ShadowSymbolRegistry[name] = symbol;
            }
            else
            {
                ShadowSymbolRegistry.Add(name, symbol);
                TotalAdditions++;
                OverflowAdditions++;
            }
        }
        
        // Look up a symbol in the shadow registry
        public static object LookupSymbol(string name)
        {
            if (!Initialized)
            {
                Initialize();
            ...
            if (ShadowSymbolRegistry.TryGetValue(name, out var symbol))
            {
                TotalRetrievals++;
                return symbol;
            }
        ...
        // Diagnostic method - print registry stats
        public static void PrintStats()
        {
            Log.Message($"[KCSG] Shadow Registry Statistics:");
            Log.Message($"  - Total Symbols: {ShadowSymbolRegistry.Count}");
            Log.Message($"  - Total Additions: {TotalAdditions}");
            Log.Message($"  - Overflow Additions: {OverflowAdditions}");
            Log.Message($"  - Total Retrievals: {TotalRetrievals}");
        }
    }
} 